 ...
 */
function isMarkdownLinkAlready(editor: Editor): boolean {
 ...
 */
function isAfterQuote(editor: Editor): boolean {
	const cursor = editor.getCursor();
 ...
 */
function isLinkedUrl(text: string): boolean {
 ...
 */
function isInsideCode(editor: Editor): boolean {
	const cursor = editor.getCursor();
 ...
 */
function toTwitterProxyUrl(url: string): string {
...
/**
 * Utility functions for URL and link state checking
 */
