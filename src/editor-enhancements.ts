 ...
 */
function cursorWithinBoundaries(
 ...
 */
function getWordBoundaries(editor: Editor): WordBoundaries {
	...
	for (const match of linksInLine) {
		if (cursorWithinBoundaries(cursor, match) && match.index !== undefined) {
			return {
