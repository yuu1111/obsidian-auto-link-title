 ...
 */
export async function fetchUrlTitleViaLinkPreview(
	url: string,
	apiKey: string,
): Promise<string> {
...
export async function fetchUrlTitle(
	...
	try {
		...
		if (settings.linkPreviewApiKey) {
			title = await fetchUrlTitleViaLinkPreview(
				url,
				settings.linkPreviewApiKey,
			);
			if (title) {
